...
func memberToProvider(m *Member) *actor.PID {
	return actor.NewPID(m.Address, "provider/"+m.ID)
}
...
// PID returns the cluster PID of where its agent can be reached.
func (m *Member) PID() *actor.PID {
	return actor.NewPID(m.Address, "cluster/"+m.ID)
}

func (m *Member) Equals(other *Member) bool {
	return m.Address == other.Address && m.ID == other.ID
}

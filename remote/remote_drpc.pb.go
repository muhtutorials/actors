...
func (c *drpcRemoteClient) Receive(ctx context.Context) (DRPCRemote_ReceiveClient, error) {
	stream, err := c.cc.NewStream(ctx, "/remote.Remote/Receive", drpcEncoding_File_remote_remote_proto{})
	if err != nil {
		return nil, err
	}
...
func (x *drpcRemote_ReceiveClient) Send(m *Envelope) error {
	return x.MsgSend(m, drpcEncoding_File_remote_remote_proto{})
}

func (x *drpcRemote_ReceiveClient) Recv() (*Envelope, error) {
	m := new(Envelope)
	if err := x.MsgRecv(m, drpcEncoding_File_remote_remote_proto{}); err != nil {
		return nil, err
	}
	return m, nil
}
...
func (x *drpcRemote_ReceiveClient) RecvMsg(m *Envelope) error {
	return x.MsgRecv(m, drpcEncoding_File_remote_remote_proto{})
}
...
func (DRPCRemoteDescription) Method(n int) (string, drpc.Encoding, drpc.Receiver, interface{}, bool) {
	switch n {
	case 0:
		return "/remote.Remote/Receive", drpcEncoding_File_remote_remote_proto{},
